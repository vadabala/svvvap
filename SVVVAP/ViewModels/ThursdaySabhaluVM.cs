...
{
    public class ThursdaySabhaluVM : INotifyPropertyChanged
    {

        // use this link to get an api_key : https://console.developers.google.com/apis/api/youtube/
        private const string ApiKey = "AIzaSyAz9IU0fzXaHIfuwub0TC6FWLRMJZzboUY";

        // doc : https://developers.google.com/apis-explorer/#p/youtube/v3/youtube.videos.list 
        private string apiUrlForChannel = "https://www.googleapis.com/youtube/v3/search?part=id&maxResults=50&channelId="
            + "UCb04_3aMVqT2uqNvp95t8Vw"
            //+ "Your_ChannelId"
            + "&key="
            + ApiKey;
        ...
        private string apiUrlForPlaylist = "https://www.googleapis.com/youtube/v3/playlistItems?part=contentDetails&maxResults=20&playlistId="
            + "PLgFCN8Mt7vN8rxYvF2xp30mj_yX4NQ_O3"
            //+ "Your_PlaylistId"
            + "&key="
            + ApiKey;
        ...
        private async Task<List<string>> GetVideoIdsFromChannelAsync()
        {
            ...
            try
            {
                ...
                foreach (var item in items)
                {
					...
					if (item.Value<JObject>("id")?.Value<string>("videoId") != null)
					{
